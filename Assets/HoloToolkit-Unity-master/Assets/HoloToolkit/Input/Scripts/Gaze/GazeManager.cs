...
namespace HoloToolkit.Unity.InputModule
{
    ...
    public class GazeManager : Singleton<GazeManager>
    {
        ...
        private List<int> debug = new List<int>();
        ...
        private void Update()
        {
            ...
            UpdateGazeInfo();
            GameObject previousFocusObject = null;
            previousFocusObject = RaycastPhysics();
            
            if(debug != null)
            {
                foreach (int k in debug)
                    Debug.Log("Layer: " + k);
            }
        ...
        private GameObject RaycastPhysics()
        {
            ...
            {
                var layerMask = ~(1 << 8);
                IsGazingAtObject = Physics.Raycast(GazeOrigin, GazeNormal, out hitInfo, MaxGazeCollisionDistance, layerMask);
            }
            else
            {
                // Raycast across all layers and prioritize
                var layerMask = ~(1 << 8); // ignore collisions with layerX
                RaycastHit? hit = PrioritizeHits(Physics.RaycastAll(new Ray(GazeOrigin, GazeNormal), MaxGazeCollisionDistance));
        ...
        private bool notHole(int layer)
        {
            Debug.Log("asd");
            return layer != 8;
        ...
        private RaycastHit? PrioritizeHits(RaycastHit[] hits)
        {
            ...
            for (int layerMaskIdx = 0; layerMaskIdx < RaycastLayerMasks.Length; layerMaskIdx++)
            {
                ...
                for (int hitIdx = 0; hitIdx < hits.Length; hitIdx++)
                {
                    ...
                    debug.Add(hit.collider.gameObject.layer);
                    if (notHole(hit.transform.gameObject.layer))
                    {
